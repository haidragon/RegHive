...
BOOL CALLBACK EditString (HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam) {
	...
	case WM_INITDIALOG: 
		...
		if (params->readonly) {
			...
			SetWindowText(hwnd,"Edit String (read only)");
		}
		params->applyEn = true;
		SetDlgItemText(hwnd,IDC_VNAME,params->name);
		SetDlgItemText(hwnd,IDC_EDITSTR,params->data);
	...
	case WM_COMMAND:
		params = (val_ed_dialog_data*)GetWindowLong(hwnd, DWL_USER);
...
BOOL CALLBACK EditMString (HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam) {
	char *c;
	int k,l,m,n;
	char s[64];
	val_ed_dialog_data *params;
	...
	case WM_INITDIALOG: 
		...
		if (params->readonly) {
			...
			SetWindowText(hwnd,"Edit Multistring (read only)");
		}
		...
		SetDlgItemText(hwnd,IDC_VNAME,params->name);
		SetDlgItemText(hwnd,IDC_EDITSTR,params->data);
		for(n=numstr=0,c=params->data;n<(int)params->data.l /*&& c[n]*/;) {
			k=strlen(c+n);
		...
		SetFocus(GetDlgItem(hwnd,IDC_EDITSTR));
		SendDlgItemMessage(hwnd,IDC_EDITSTR,EM_SETSEL,0,-1);
		sprintf(s,"String %i of %i:",cursel,
			SendDlgItemMessage(hwnd,IDC_SLIST,LB_GETCOUNT,0,0));
...
setsel:	cursel=SendDlgItemMessage(hwnd,IDC_SLIST,LB_GETCURSEL,0,0);
		if (cursel<0) cursel=0; else {
			...
			char *c1;
			c=(char*)malloc(l+10); c1=(char*)malloc(l1+10);
			*c=*c1=0;
			GetDlgItemText(hwnd,IDC_EDITSTR,c,l+9);
			SendDlgItemMessage(hwnd,IDC_SLIST,LB_GETTEXT,cursel,(LPARAM)c1);
			int t = params->applyEn;
			params->applyEn = 2;
			if (strcmp(c,c1)) SetDlgItemText(hwnd,IDC_EDITSTR,c1);
			params->applyEn = t;
			free(c); free(c1);
		}
		sprintf(s,"String %i of %i:",cursel,
...
BOOL CALLBACK EditDWORD (HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam) {
	...
	case WM_INITDIALOG: 
		...
		*((DWORD*)params->newdata.c) = cval = *((DWORD*)params->data.c);
		if (params->data.l != 4) SetWindowText(hwnd, "Edit false DWORD Value?!");
		SetDlgItemText(hwnd,IDC_VNAME,params->name);
		if (params->readonly) {
...
BOOL CALLBACK EditBinary (HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam) {
	val_ed_dialog_data *params;
	char s[64];
	switch (msg) {
	case WM_INITDIALOG: 
		...
		sprintf(s,"Edit %sBinary %s", params->flag1?"value as ":"",params->readonly?" (read only)":"");
		SetWindowText(hwnd,s);
		params->applyEn = 1;
		SetDlgItemText(hwnd,IDC_VNAME,params->name);
